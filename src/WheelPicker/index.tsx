...
const WheelPicker: React.FC<IProps> = ({
  options,
  defaultIndex = 0,
  itemHeight = 40,
  scrollEnd,
    ...
    switch (event.type) {
      ...
      case "touchend": {
        ...
        const oversizeBorder =
          -(
            // @ts-ignore
            sliderRef.current.getElementsByTagName("li").length -
            selectLineOffsetNum -
            1
          ) * itemHeight;
      ...
      case "touchmove":
        event.preventDefault();
        movePosition(updateCurDistance() + event.touches[0].clientY - oldMoveY);
  ...
  const renderWheels = () => (
    ...
    >
      <div className="grayLayer top-0 left-0 right-0 bottom-0 block pointer-events-none" />
